...
var httpReqPatternFormat = "%sReqPattern"
		...
		if !handlerFound {
			...
			if g.gorillaMux {
				st = jen.Id("m").Dot("Methods").Call(
					...
					jen.Id(fmt.Sprintf(httpReqPatternFormat, utils.ToLowerFirstCamelCase(m.Name))),
				).Dot("Handler").Call(
...
func newGenerateHTTPTransportBase(name string, gorillaMux bool, serviceInterface parser.Interface, methods []string, allMethods []parser.Method) Gen {
	...
	t.httpPathFilePath = path.Join(t.destPath, viper.GetString("gk_http_path_file_name"))
	t.srcFile = jen.NewFilePath(t.destPath)
	t.InitPg()
	t.fs = fs.Get()
	return t
...
func (g *generateHTTPTransportBase) generateCmdPathVar() (err error) {
	g.srcFile = jen.NewFilePath(g.destPath)
	g.InitPg()
	g.srcFile.PackageComment("THIS FILE IS AUTO GENERATED BY GK-CLI DO NOT EDIT!!")
	...
	for _, path := range g.cmdPaths {
		...
		reqPathVar := fmt.Sprintf(httpReqPatternFormat, utils.ToLowerFirstCamelCase(path))
